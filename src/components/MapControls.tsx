...
const MapControls = () => {
  ...
  return (
    <Card className="absolute top-16 right-4 p-3 space-y-2 z-[2000] bg-white/95 backdrop-blur-sm border shadow-lg min-w-0">
      <div className="flex flex-col space-y-2 items-center">
        <Button
          variant={showTransit ? "default" : "outline"}
          size="sm"
          onClick={toggleTransit}
          className="flex items-center gap-2 w-full justify-center text-xs px-3 py-1.5"
        >
          <Train size={14} />
          Transit Lines
        </Button>
      ...
      <div className="text-xs text-gray-600 space-y-2 w-48">
        ...
        <div className="pt-2 border-t border-gray-200">
          {showRestaurants ? (
            <div className="text-xs text-gray-500 p-2 bg-gray-50 rounded text-center">
              Zoom in (level 12+) to load restaurants
            </div>
          ) : (
